...
for arch in ['mlp']:
    ...
    for dataset in ['yahoo']:
        ...
        with open('tables-paired-test/' + arch + '-' + dataset + '.tsv', 'w+') as fo:
            for i in range(len(methods_risk)):
                fo.write(methods_risk[i] + "\t")
            ...
            for i in range(len(methods_def)):
                ...
                write = True
                if i % 2 == 0:
                    if np.mean(get_arr(home + '/' + methods_def[i + 1], 'lndcg_10')) > np.mean(get_arr(home + '/' + methods_def[i], 'lndcg_10')):
                        write = False
                else:
                    if np.mean(get_arr(home + '/' + methods_def[i - 1], 'lndcg_10')) > np.mean(get_arr(home + '/' + methods_def[i], 'lndcg_10')):
                        write = False
                if write:
                    fo.write(methods_def[i] + "\t")
                    for metric in ['lndcg_10', 'lndcg_5', 'georisk5lndcg_10', 'georisk5lndcg_5']:
                        method_best = get_best_def_in_metric(metric)
