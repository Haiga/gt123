...
for arch in ['', 'mlp']:
    ...
    for dataset in ['mq2007']:
        ...
        # dataset = 'yahoo'
        if arch == '':
            home = fr'D:\Colecoes\experimento_loss_risk\tables-apresent-3\{dataset}'
        else:
            home = fr'D:\Colecoes\experimento_loss_risk\tables-apresent-3\{arch}\{dataset}'
        ...
        def get_arr(file, metric_index):
            ...
            # print(len(np.unique(pd.read_csv(file + ".tsv", sep="\t")[metric_index].values)))
            if len(np.unique(pd.read_csv(file + ".tsv", sep="\t")[metric_index].values)) > 5:
                return pd.read_csv(file + ".tsv", sep="\t")[metric_index].values
        ...
        with open('tables-paired-test3/' + arch + '-' + dataset + '.tsv', 'w+') as fo:
            for i in range(len(methods_risk)):
                fo.write(getName(methods_risk[i]) + "\t")
                for metric in ['georisk5lndcg_10', 'georisk5lndcg_5']:
