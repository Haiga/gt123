...
for dataset in ['mq2007', 'yahoo', 'web10k']:
    for tt in ['att', 'mlp']:
        ...
        # home = r"D:\Colecoes\experimento_loss_risk\tables\overall2"+f"\\{tt}"+f"\\{dataset}"
        # home = r"D:\Colecoes\experimento_loss_risk\tables\overall2"+f"\\{tt}"+f"\\{dataset}"
        home = r"D:\Projetos Códigos\PycharmProjects\pythonProject\merged2"+f"\\{tt}"+f"\\{dataset}"
        # home = r"D:\Projetos Códigos\PycharmProjects\pythonProject\merged"+f"\\{tt}"+f"\\{dataset}"
        ...
        for metric in metrics:
            mat = []
            if "georisk" in metric: continue
            for method in methods_dics:
                mat.append(methods_dics[method][metric])
            geoMeasure = getGeoRisk(np.array(mat).T, alpha=5)
            cont = 0
            for method in methods_dics:
                r = [geoMeasure[cont]]*len(methods_dics[method][metric])
                methods_dics[method]["georisk5" + metric].extend(r)
                cont += 1
        g_metrics = []
        for metric in metrics:
