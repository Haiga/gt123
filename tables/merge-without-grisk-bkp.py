...
home = r'D:\Colecoes\experimento_loss_risk\tables\overall\att\yahoo'

# methods = ['geoRiskSpearmanLoss', 'geoRiskListnetLoss', 'spearmanLoss', 'lambdaLoss', 'listNet', 'ordinal',
#            'pointwise_rmse']

methods = ['extgeoRiskSpearmanLoss', 'extgeoRiskListnetLoss', 'geoRiskSpearmanLoss', 'geoRiskListnetLoss', 'spearmanLoss', 'lambdaLoss', 'listNet', 'ordinal',
           'pointwise_rmse']
...
def saveFile(file, data):
    ...
    for alg in data:
        ...
        for i in range(1, len(alg) - 1):
            alg[i][0] = round(alg[i][0], 4)
            # arq.write((f'\t{alg[i][0]}\t{alg[i][1]}\t{alg[i][2]}').replace('.', ','))
            # print((f'\t{alg[i][0]}\t{alg[i][1]}\t{alg[i][2]}').replace('.', ','), end='')
            arq.write((f'\t{alg[i][0]:7.4f} ({alg[i][2]})').replace('.', ','))
            print((f'\t{alg[i][0]:7.4f} ({alg[i][2]})').replace('.', ','), end='')
