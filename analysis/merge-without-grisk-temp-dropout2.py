...
home = r"D:\Colecoes\experimento_loss_risk\tuning-multilayer\resultados-web10k-multilayer5\results"
home = r"D:\Colecoes\experimento_loss_risk\tuning-datay-mq2007\resultados-mq2007-tuning\results"
home = r"D:\Colecoes\experimento_loss_risk\tuning-datay-mq2007\datay-tuning\results"
...
for size in [[256, 128, 64], [64, 64], [128, 64], [64, 128, 64]]:
    for input_norm in [True, False]:
        for act in ["Sigmoid", ""]:
            for fold in [0]:
                for seed in [42]:
                    for lr in [0.001, 0.0001]:
                        n_fold = str(fold + 1)
                        for loss in ['geoRiskSpearmanLoss', 'geoRiskListnetLoss', 'spearmanLoss', 'lambdaLoss',
                                     'listNet', 'pointwise_rmse']:
                        # for loss in ['geoRiskSpearmanLoss', 'geoRiskListnetLoss', 'spearmanLoss', 'lambdaLoss',
                        #              'listNet', 'pointwise_rmse', 'ordinal']:
                            name = ""
                            for s in size:
                                name += str(s) + "."
                            name += "-" + str(act)
