...
for line in lines:
    ...
    for j in range(len(h) - 1):
        if h[j] not in infos[cols[j]]:
            ...
            infos[cols[j]][h[j]]["max"] = 0
        infos[cols[j]][h[j]]["sum"] += float(h[-1])
        infos[cols[j]][h[j]]["tot"] += 1
        infos[cols[j]][h[j]]['max'] = max(infos[cols[j]][h[j]]['max'], float(h[-1]))

for fact in infos:
    for level in infos[fact]:
        # if 'sum' == level or 'tot' == level or 'mean' == level or 'max' == level: continue
        infos[fact][level]['mean'] = infos[fact][level]['sum'] / infos[fact][level]['tot']
        # infos[fact]['sum'] = infos[fact][level]['sum']
        # infos[fact]['tot'] = infos[fact][level]['tot']
    # infos[fact]['mean'] = infos[fact]['sum'] / infos[fact]['tot']
...
for fact in att:
    ...
    informations[fact]["f"] = scipy.stats.f.cdf(0.99, len(infos[fact]) - 1, tam)
