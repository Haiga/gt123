    ...
    for v in [5]:
        for fold in [0]:
            for seed in [42]:
                ...
                for lr in [0.001, 0.0005, 0.0001]:
                    # for vu in [0.001, 0.0001, 0.00001, 0.1####, 0.01]:
                    for vu in [0.001]:
                        n_fold = str(fold + 1)
                        for loss in ['spearmanLoss']:
                            ...
                            data['loss']['name'] = "geoRiskSpearmanLoss"
                            data['loss']['args'] = {}
                            data['loss']['args']['corr'] = 1  ###
                            data['training']['epochs'] = 60
                            data['model']['transformer']['d_ff'] = 256###nt
                            data['model']['transformer']['N'] = 2###nt
                            data['model']['transformer']['h'] = 2###nt
                            ...
                            try:
                                ...
                                myrun(loss + "fold" + n_fold + "-" + name,
                                      ...
                                      useCuda=False, seed=seed, num_baselines=2)
