...
def run(id, jobdir=None, configfilename=None, useCuda=True, seed=42, num_baselines=5, path_base=None):
    ...
    if configfilename is None:
        args.config_file_name = "localtemp_config.json"
    else:
...
if __name__ == "__main__":
    # run("id5", num_baselines=5)

    datasetpaths = {
        "mq2007": "D:\\Colecoes\\BD\\MQ2007\\MQ2007\\Fold1",
        "web10k": "D:\\Colecoes\\BD\\web10k-norm\\Fold1",
        "2003": "D:\\Colecoes\\BD\\2003_td_dataset-norm\\Fold2",
        "yahoo": "D:\\Colecoes\\BD\\yahoo-c14\\l2r",
    }
    ...
    #     try:
    #         with open(
    #                 'localtemp_config.json') as json_file:
    #             data = json.load(json_file)
    #
    #         data['training']['epochs'] = 25
    #
    #         data['data']['path'] = datasetpaths[dataset]
    #
    #         with open("localtemp_config.json",
    #                   'w') as outfile:
    #             json.dump(data, outfile)
    #
    #         run("analise-external-teste2" + dataset, num_baselines=0)
    #     except:
    #         mmmmm = 0

    for loss in ["pointwise_rmse", "listNet", "lambdaLoss", "spearmanLoss", "ordinal"]:
        for dataset in ["mq2007", "web10k", "yahoo"]:
    # for loss in ["pointwise_rmse"]:
    #     for dataset in ["mq2007"]:
        ...
        # for dataset in []:
            try:
                ...
                data['loss']['args'] = {}
                if "ordinal" in loss:
                    data['loss']['args']['normalized'] = 0
                    data['loss']['args']['n'] = 5
                    data['model']['post_model']['d_output'] = 5  # multi saidas para ordinal

                data['data']['path'] = datasetpaths[dataset]
                data['training']['epochs'] = 1#50
                data['loss']['name'] = loss
                if "yahoo" in dataset:
                    data['training']['epochs'] = 1#25
                with open("localtemp_config.json",
                          'w') as outfile:
                    json.dump(data, outfile)

                run("analise" +loss+ dataset, num_baselines=2)#5
            except:
                mmm= 0