...
logger = get_logger()
preds_of_epoch_current = [0]*100
preds_of_epoch_current_total = [1]*100

def loss_batch(model, loss_func, xb, yb, yp_baseline, indices, gradient_clipping_norm, opt=None, num_baselines=5, total_of_calculated_corr_in_epoch=0, var_epoch=0):
    mask = (yb == PADDED_Y_VALUE)
    xb[torch.isnan(xb)] = 0#Added to avoid NaN in X
    if num_baselines != 0:
        baseline_predicts = []
    ...
    # if num_baselines == 0:
    #     if total_of_calculated_corr_in_epoch != -1:
    #         total_users_batch = 20
    #         num_baseline_dropouts2 = len(baseline_predicts.T) - 1
    #         for drop1pred in range(num_baseline_dropouts2):
    #             for drop2pred in range(num_baseline_dropouts2):
    #                 if drop1pred != drop2pred:
    #                     for i in range(total_users_batch):
    #
    #                         try:
    #                             corr_total_local = abs(scipy.stats.pearsonr(baseline_predicts.T[drop1pred+1].T[i]
    #                                                       .cpu().detach().numpy(),
    #                                                       baseline_predicts.T[drop2pred+1].T[i]
    #                                                       .cpu().detach().numpy())[0])
    #                         except:
    #                             corr_total_local = -2
    #
    #                         if corr_total_local != -2:
    #                             preds_of_epoch_current_total[var_epoch] += 1
    #                             preds_of_epoch_current[var_epoch] += corr_total_local
    #
    #                             total_of_calculated_corr_in_epoch += 1
    #
    #         # total_users_batch = 5
    #
    #
    #
    # else:
    #     if total_of_calculated_corr_in_epoch != -1:# and random.randint(0, 20) == 5:
    #         total_users_batch = 20
    #         # total_users_batch = 5
    #         num_baseline_dropouts2 = 5
    #         for i in range(total_users_batch):
    #             for drop1pred in range(num_baseline_dropouts2):
    #                 reshapertn = baseline_predicts[drop1pred].cpu().detach().numpy()
    #                 if len(reshapertn) == 20:##
    #                     for drop2pred in range(num_baseline_dropouts2):
    #                         if drop1pred != drop2pred:
    #                             preds_of_epoch_current_total[var_epoch] += 1
    #                             preds_of_epoch_current[var_epoch] += \
    #                                 abs(scipy.stats.spearmanr(reshapertn[i],
    #                                                       baseline_predicts[drop2pred].cpu().detach().numpy()[i])[0])
    #                             total_of_calculated_corr_in_epoch += 1
...
def compute_test2(metrics, model, dl, dev, output_dir):
    ...
    #path_correlations = os.path.join(output_dir, "correlations.txt")
    with open(path_predictions, 'w') as fo_predictions:
        # with open(path_correlations, 'w') as fo_correlations:
        all_results_metric = {}
...
def fit(id, epochs, model, loss_func, optimizer, scheduler, train_dl, valid_dl, test_dl, config,
    ...
    # file_object.write("\n" + id + "\n")
    path_correlations = os.path.join(output_dir, "correlations.txt")
    path_time = os.path.join(output_dir, "total-time.txt")
    with open(path_correlations, 'w') as fo_correlations:
        ...
        for epoch in range(epochs):
            ...
            total_of_calculated_corr_in_epoch = 0

            train_losses, train_nums = zip(
                *[loss_batch(model, loss_func, xb.to(device=device), yb.to(device=device), yp_baseline.to(device), indices.to(device=device),
                             gradient_clipping_norm, optimizer, num_baselines=num_baselines,
                             total_of_calculated_corr_in_epoch=total_of_calculated_corr_in_epoch, var_epoch=epoch) for
                  xb, yb, yp_baseline, indices in train_dl])
            train_loss = np.sum(np.multiply(train_losses, train_nums)) / np.sum(train_nums)
            train_metrics = compute_metrics(config.metrics, model, train_dl, device)

            #save values
            if total_of_calculated_corr_in_epoch == 0:
                total_of_calculated_corr_in_epoch = 1
            fo_correlations.write(str(preds_of_epoch_current[epoch]/preds_of_epoch_current_total[epoch]) + "\n")
            model.eval()
            with torch.no_grad():
                val_losses, val_nums = zip(
                    *[loss_batch(model, loss_func, xb.to(device=device), yb.to(device=device), yp_baseline.to(device), indices.to(device=device),
                                 gradient_clipping_norm, num_baselines=num_baselines, total_of_calculated_corr_in_epoch=-1) for
            ...
            if early_stop.stop_training(epoch):
                ...
                break
        end_time = time.time()
        with open(path_time, 'w') as fo_time_execution:
            fo_time_execution.write(str(end_time - started_time) + "\n")
